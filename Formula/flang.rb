...
class Flang < Formula
  ...
  url "https://github.com/digitable-lol/flang/releases/download/v0.7.3/flang-0.7.3-c.tar.gz"
  ...
  # не удалялись (запись запрещена), просто пропущены.
  #
  # Число ниже — с версией 0.7.3, и оно НЕ пересчитано вручную: взято
  # напрямую из sha256sum самого конвейера — прогон release.yml через
  # workflow_dispatch на этом дереве ДО метки (шаг выкладки на нём не идёт,
  # это сухая проба; сверка хеша упала нарочно, старое число было
  # заведомо чужим, и её же ошибка назвала настоящий хеш). Больше не гадаю
  # локально — конвейер сам себе источник правды для этого числа.
  # Прежде здесь стояло ddf21617… от выпуска 0.7.0, чей
  # архив был байт в байт тем же кодом, но с #define FLANG_VERSION "0.6.2" —
  # строкой, которую забыли бампнуть вместе с package.json. Отсюда и хотфикс:
  # release-versions/9959.md.
  # b8d82b1c… от выпуска 0.6.2 стоял ещё раньше, 2 995 862 байта.
  sha256 "c101dae42007afa697893d68358dea1d9d220c76839555147676bd791f435159"
